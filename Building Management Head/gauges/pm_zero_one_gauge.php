<?php     
    ...
    if (isset($_POST['room_number'])) {
        ...
        while ($row = mysqli_fetch_assoc($result_table)){
            $pm_zero_one_level[] = $row['pm_zero_one'];
        }
    }
?>
...
<body>
    <span class="gauge-title">Particulate Matter 0.1 (Latest)</span>
    <div id="pm-zero-one-gauge" class="gauge"></div>
    <script>
        $(document).ready(function() {
            // Configuration options for the gauge
            var gaugeConfig = {
                id: 'pm-zero-one-gauge',
                value: '<?php echo isset($pm_zero_one_level[0]) ? $pm_zero_one_level[0] : 0 ; ?>',
                symbol: ' µg/m³',      
