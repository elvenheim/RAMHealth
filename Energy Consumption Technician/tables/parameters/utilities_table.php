<?php     
    ...
    if (isset($_POST['submit']) && isset($_POST['ec_sensor'])) {
        ...
        $sql = "SELECT acu.*, eas.arduino_bldg_floor, eas.arduino_room_num, bf.bldg_floor_name
                FROM ec_param_util_data acu
                JOIN ec_arduino_sensors eas ON acu.ec_sensor_util_id = eas.ec_arduino_sensors_id
                ...
                INNER JOIN (SELECT ec_sensor_util_id, MAX(CONCAT(ec_util_date, ' ', ec_util_time)) AS max_datetime
                        ...
                        ON acu.ec_sensor_util_id = latest.ec_sensor_util_id 
                AND CONCAT(acu.ec_util_date, ' ', acu.ec_util_time) = latest.max_datetime
                WHERE eas.ec_arduino_sensors_id IN ('$acuSensors')
                ORDER BY acu.ec_sensor_util_id ASC
        ...
        if (mysqli_num_rows($result_table) == 0) {
            ...
            while ($row = mysqli_fetch_assoc($result_table)){
                ...
                echo "<td>" . $row['ec_util_current']  . ' amperes' . "</td>";
                echo "</tr>";
            }   
        }
    }
