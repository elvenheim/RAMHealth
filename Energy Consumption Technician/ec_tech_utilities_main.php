				...
				<div class = "table-button">
					...
					<form class="import-table" method="POST" enctype="multipart/form-data" action="../scripts/import_table_ec.php">
						<label class="import-btn">
							...
							<input type="hidden" id="table_name" name="table_name" value="ec_param_util_data">
							<input type="file" name="csv_file" style="display: none;" required accept=".csv" onchange="submitForm()">
						</label>
                    </form>
					<button id="download-table" class="download-table" onclick="downloadUtilExcelTable()">
                        <span class="fas fa-download"></span> Export </button>
				</div>
				<table id = 'utility-parameters-table' class = 'utilities-table'>
					<thead>
						<tr>
							<th><a href="#arrange-floor" onclick="sortUtilTable(0)">Floor<span class="sort-indicator">&#x25BC;</span></a></th> 
							<th><a href="#arrange-room-number" onclick="sortUtilTable(1)">Room Number<span class="sort-indicator">&#x25BC;</span></a></th>
                            <th><a href="#arrange-sensor-id" onclick="sortUtilTable(2)">Sensor ID<span class="sort-indicator">&#x25BC;</span></a></th>
                            <th><a href="#arrange-date" onclick="sortUtilTable(3)">Date<span class="sort-indicator">&#x25BC;</span></a></th>
                            <th><a href="#arrange-time" onclick="sortUtilTable(4)">Time<span class="sort-indicator">&#x25BC;</span></a></th>
                            <th><a href="#arrange-current" onclick="sortUtilTable(5)">Current<span class="sort-indicator">&#x25BC;</span></a></th>
						</tr>
					</thead>
					<tbody id = 'table-body'>
						<?php include '../Energy Consumption Technician/tables/parameters/utilities_table.php'?>
					</tbody> 
				</table>
				<?php include '../Energy Consumption Technician/EC Tech Design/Pagination/utilities_pagination.php'; ?>
			</div>
		</div>
    </div>
</body>
